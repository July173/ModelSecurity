...
namespace Web.Controllers
{
    ...
    public class AprendizProgramController : ControllerBase
    {
        ...
        /// <summary>
        /// Elimina un AprendizProgram por su ID.
        /// </summary>
        [HttpDelete("{id}")]
        ...
        public async Task<IActionResult> DeleteAprendizProgram(int id)
        {
            try
            {
                var result = await _aprendizProgramBusiness.DeleteAprendizProgramAsync(id);
                return Ok(result);
            }
            catch (ValidationException ex)
            {
            ...
            {
                _logger.LogInformation(ex, "AprendizProgram no encontrado al eliminar: {Id}", id);
                return NotFound(new { message = ex.Message });
            }
            catch (ExternalServiceException ex)
            {
                _logger.LogError(ex, "Error al eliminar AprendizProgram");
                return StatusCode(500, new { message = ex.Message });
            }
        }
