...
namespace Web.Controllers
{
    ...
    public class AprendizProcessInstructorController : ControllerBase
    {
        ...
        /// <summary>
        /// Elimina un AprendizProcessInstructor por su ID.
        /// </summary>
        [HttpDelete("{id}")]
        ...
        public async Task<IActionResult> DeleteAprendizProcessInstructor(int id)
        {
            try
            {
                var result = await _aprendizProcessInstructorBusiness.DeleteAprendizProcessInstructorAsync(id);
                return Ok(result);
            }
            catch (ValidationException ex)
            {
            ...
            {
                _logger.LogInformation(ex, "AprendizProcessInstructor no encontrado al eliminar: {Id}", id);
                return NotFound(new { message = ex.Message });
            }
            catch (ExternalServiceException ex)
            {
                _logger.LogError(ex, "Error al eliminar AprendizProcessInstructor");
                return StatusCode(500, new { message = ex.Message });
            }
        }
